...
export class AuthService {
  ...
  public changeIsSeller(isSeller: boolean) {
  ...
  public async registerForNewCompany(email: string, name: string, password: string, companyName: string): Promise<any> {
    if (!(email?.trim() && name?.trim() && password?.trim() && companyName?.trim())) {
      throw new Error('Missing input fields');
    }
    const body = {fullname: name, email: email, company: { name: companyName }, role: { name: ROLES.Admin }, password: password};
    const registrationResponse = await this.http.put(`${environment.apiUrl}users/register`, body, { headers: this.headersSkipAuth }).toPromise();
    return registrationResponse;
  }

  public async registerForExistingCompany(email: string, name: string, password: string, companyId: number): Promise<any> {
    if (!(email?.trim() && name?.trim() && password?.trim() && companyId)) {
      throw new Error('Missing input fields');
    }
    const body = {fullname: name, email: email, role: { name: ROLES.Member }, password: password };
    const registrationResponse = await this.http.put(`${environment.apiUrl}companies/${companyId}/users`, body, { headers: this.headersSkipAuth }).toPromise();
    return registrationResponse;
  }

  public async corbadoSessionVerify(corbadoSessionToken: string) {
    const loggedInUser = await this.http.get<AuthUser>(`${environment.apiUrl}auth/sessionVerify?corbadoSessionToken=${corbadoSessionToken}`).toPromise();
    this.user = loggedInUser;
    if (!loggedInUser?.idToken) throw new Error(`cannot fetch authentication token ${loggedInUser}!`);
    loggedInUser.last_refresh = +Date.now();
    this.saveUserCookie();
